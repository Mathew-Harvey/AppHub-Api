...
router.get('/status', auth, async (req, res) => {
  try {
    ...
    const row = u.rows[0];
    const userPlan = bypassPlan ? 'power' : (row.plan || 'free');
...
// POST /api/subscription/checkout — authenticated user subscribes or changes plan
router.post('/checkout', auth, async (req, res) => {
  ...
  try {
    ...
    let customerId = u.rows[0].stripe_customer_id;
...
// POST /api/subscription/portal — billing portal for managing subscription
router.post('/portal', auth, async (req, res) => {
  ...
  try {
    ...
    if (u.rows.length === 0) return res.status(404).json({ error: 'User not found' });
    if (!u.rows[0].stripe_customer_id) {
    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (userId && session.subscription) {
          ...
          // Update ALL user records with this email across all workspaces
          const result = await pool.query(
            `UPDATE users SET plan = $1, stripe_customer_id = $2, stripe_subscription_id = $3, updated_at = NOW()
             WHERE LOWER(email) = $4 AND is_active = true
             RETURNING id`,
            [plan, session.customer, session.subscription, email]
          );
          ...
          if (result.rowCount > 0) {
            ...
            console.log(`${result.rowCount} user record(s) for ${email} upgraded to ${plan}`);
          } else {
            console.warn(`Checkout: no user found for email ${email}`);
          }
        }
        break;
      }
      ...
      case 'customer.subscription.updated': {
        ...
        const result = await pool.query(
          `UPDATE users SET plan = $1, stripe_subscription_id = $2, updated_at = NOW()
           WHERE stripe_customer_id = $3 AND is_active = true
           RETURNING id`,
          [plan, sub.id, customerId]
        );

        if (result.rowCount > 0) {
          console.log(`${result.rowCount} user(s) with customer ${customerId} updated -> ${plan}`);
        }
        break;
      }
      ...
      case 'customer.subscription.deleted': {
        ...
        if (result.rowCount > 0) {
          console.log(`${result.rowCount} user(s) with customer ${customerId} downgraded to free`);
        }
        break;
      }
