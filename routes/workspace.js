...
router.post('/invite', auth, adminOnly, async (req, res) => {
  ...
  try {
    const existing = await pool.query(
      ...
      [cleanEmail, req.user.workspaceId]
    );
    ...
    const result = await pool.query(
      ...
      [req.user.workspaceId, cleanEmail, req.user.id]
    );
...
router.get('/invitations', auth, adminOnly, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT i.id, i.email, i.accepted, i.created_at
       FROM invitations i
...
router.delete('/invite/:id', auth, adminOnly, validateId, async (req, res) => {
  try {
    const result = await pool.query(
      'DELETE FROM invitations WHERE id = $1 AND workspace_id = $2 RETURNING id',
...
router.put('/members/:id/role', auth, adminOnly, validateId, async (req, res) => {
  ...
  try {
    const member = await pool.query(
      'SELECT id FROM users WHERE id = $1 AND workspace_id = $2 AND is_active = true',
      [req.params.id, req.user.workspaceId]
    );
...
// DELETE /api/workspace/members/:id — deactivate member (admin only)
